...
public class DataBaseSource {
    ...
    public Cursor getGlMjerenja(String danStr){
        ...
        String upit =  "Select id_rez,CAST(substr(vrijeme,instr(vrijeme,' ')+1,2) as real) + CAST(substr(vrijeme,instr(vrijeme,':')+1,2) as real)/60 as vrijeme," +
                    "izmjereno from REZULTATI WHERE substr(vrijeme,1,10) = '" + danStr + "' order by 2;";
    ...
    public Cursor getSvaMjerenja(){
        String upit = "SELECT vrijeme, izmjereno, id_attr_fk, tip_mj from REZULTATI order by 1";
        Cursor cursor = db.rawQuery(upit,null);
        return cursor;
    }

    public Cursor getSvaTerapija(){
        String upit = "SELECT jed_insl, jed_kor, poz_prijem_ter, dan||' '||vrijeme as vrijeme from TERAPIJA order by 4";
        Cursor cursor = db.rawQuery(upit,null);
        return cursor;
    }

    public Cursor getSveBiljeske(){
        String upit = "SELECT biljeska, datum from DANI_BIL";
        Cursor cursor = db.rawQuery(upit,null);
        return cursor;
    }
    ...
    public void parseJsonIntoDb(String parseJson) throws JSONException {
        ...
        jsonMjerenja = json.getJSONArray("mjerenja");
        jsonTerapija = json.getJSONArray("terapija");
        jsonBiljeske = json.getJSONArray("biljeske");

        for(int m = 0; m < jsonMjerenja.length(); m++){
            JSONObject currMea = jsonMjerenja.getJSONObject(m);
            insertVal.put(COLUMN_VRIJEME, currMea.getString("vrijeme"));
            insertVal.put(COLUMN_ID_ATTR_FK, currMea.getInt("atr_id_atr"));
            insertVal.put(COLUMN_IZMJERENO, currMea.getDouble("izmjereno"));
            insertVal.put(COLUMN_TIP_MJ, currMea.getInt("tip_mj"));
            insertVal.put(COLUMN_BACKED_UP, 1);
            if(alreadyExists(TABLE_REZULTATI, COLUMN_VRIJEME, currMea.getString("vrijeme")) == false) {
        ...
        for(int b = 0; b < jsonBiljeske.length(); b++){
            ...
            insertVal.put(COLUMN_BILJESKA,  currBiljeska.getString("biljeska"));
            insertVal.put(COLUMN_DATUM, currBiljeska.getString("datum"));
            insertVal.put(COLUMN_BACKED_UP, 1);
            if(alreadyExists(TABLE_DANI_BIL, COLUMN_DATUM, currBiljeska.getString("datum")) == false) {
        ...
        for(int t = 0; t < jsonTerapija.length(); t++){
            ...
            String[] datumVrijeme = currTer.getString("dat_vrijem").split(" ");
            insertVal.put(COLUMN_JED_INSL, currTer.getInt("jed_insl"));
            insertVal.put(COLUMN_JED_KOR, currTer.getInt("jed_kor"));
            insertVal.put(COLUMN_POZ_PRIJEM_TER, currTer.getInt("poz_prijem_ter"));
            insertVal.put(COLUMN_DAN, datumVrijeme[0]);
            insertVal.put(COLUMN_VRIJEME, datumVrijeme[1]);
            insertVal.put(COLUMN_BACKED_UP, 1);
            if(alreadyExists(TABLE_TER, COLUMN_DAN+"||' '||"+COLUMN_DAN, currTer.getString("dat_vrijem")) == false) {
